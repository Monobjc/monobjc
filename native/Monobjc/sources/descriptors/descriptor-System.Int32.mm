 ...
 */
static MonoObject *__marshal_from_native_for_System_Int32(MonobjcTypeDescriptor *descriptor, void *ptr, boolean_t native) {
    //LOG_DEBUG(MONOBJC_DOMAIN_MARSHALLING, "__marshal_from_native_for_System_Int32(%p)", ptr);

    // Box the value directly into a managed object
    MonoObject *obj = mono_value_box(mono_domain_get(), mono_get_int32_class(), ptr);
    return obj;
}
...
MonobjcTypeDescriptor *monobjc_create_descriptor_for_System_Int32() {
    LOG_DEBUG(MONOBJC_DOMAIN_MARSHALLING, "monobjc_create_descriptor_for_System_Int32");
    
    // Allocate the descriptor structure
    MonobjcTypeDescriptor *descriptor = g_new0(MonobjcTypeDescriptor, 1);
    
    // Set the type
    descriptor->type = mono_class_get_type(mono_get_int32_class());
    descriptor->boxed = 1;

    // Set the various marshaling functions
    descriptor->alloc_native_storage = __alloc_native_storage_for_System_Int32;
    descriptor->marshal_from_native = __marshal_from_native_for_System_Int32;
    descriptor->marshal_to_native = __marshal_to_native_for_System_Int32;
    descriptor->free_native_storage = __free_native_storage_for_System_Int32;
    ...
    descriptor->foreign_type = &ffi_type_sint32;
